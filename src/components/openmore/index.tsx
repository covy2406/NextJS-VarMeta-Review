  ...
  return (
    // <div>
      <div className='container mx-auto py-[8.125rem]'>
        {dataMore.map((item) => (
          <>
            <div key={item.id} onClick={() => handleOpen(item.id)} className='bg-[#f6f9fe] mb-4 flex p-5 rounded-md justify-between text-lg font-medium cursor-pointer'>
              <p>{item.title}</p>
              {open === item.id ? <LuMinus /> : <LuPlus />}
            </div>
            {open === item.id && (
              <div className='text-base font-normal p-5 text-[#565656]'>
